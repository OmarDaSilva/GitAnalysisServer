// This is a generated file, modify: generate/templates/templates/struct_content.cc

// generated from struct_content.cc
#include <nan.h>
#include <string.h>
#ifdef WIN32
#include <windows.h>
#else
#include <unistd.h>
#endif // win32

extern "C" {
  #include <git2.h>
 }

#include <iostream>
#include "../include/nodegit.h"
#include "../include/lock_master.h"
#include "../include/functions/copy.h"
#include "../include/repository_init_options.h"
#include "nodegit_wrapper.cc"

 
using namespace v8;
using namespace node;
using namespace std;

 
ConfigurableGitRepositoryInitOptions::ConfigurableGitRepositoryInitOptions(nodegit::Context *nodegitContext)
  : nodegit::ConfigurableClassWrapper<GitRepositoryInitOptionsTraits>(nodegitContext)
{
     git_repository_init_options  wrappedValue = GIT_REPOSITORY_INIT_OPTIONS_INIT;
    this->raw = (git_repository_init_options*) malloc(sizeof(git_repository_init_options ));
    memcpy(this->raw, &wrappedValue, sizeof(git_repository_init_options ));
 }

ConfigurableGitRepositoryInitOptions::~ConfigurableGitRepositoryInitOptions() {
              delete this->raw->workdir_path;
          delete this->raw->description;
          delete this->raw->template_path;
          delete this->raw->initial_head;
          delete this->raw->origin_url;
   }

nodegit::ConfigurableClassWrapper<GitRepositoryInitOptionsTraits>::v8ConversionResult ConfigurableGitRepositoryInitOptions::fromJavascript(nodegit::Context *nodegitContext, v8::Local<v8::Value> input) {
  if (!input->IsObject()) {
    return {
      "Must pass object for ConfigurableGitRepositoryInitOptions"
    };
  }

  Nan::HandleScope scope;
  v8::Local<v8::Object> inputObj = input.As<v8::Object>();
  std::shared_ptr<ConfigurableGitRepositoryInitOptions> output(new ConfigurableGitRepositoryInitOptions(nodegitContext));

  // unpack the data into the correct fields
         {
          v8::Local<v8::Value> maybeNumber = nodegit::safeGetField(inputObj, "version");
          if (!maybeNumber.IsEmpty() && !maybeNumber->IsUndefined() && !maybeNumber->IsNull()) {
            if (!maybeNumber->IsNumber()) {
              return {
                "Must pass Int32 to version"
              };
            }

            output->raw->version = static_cast<unsigned int>(maybeNumber->Int32Value(Nan::GetCurrentContext()).FromJust());
          }
        }
           {
          v8::Local<v8::Value> maybeNumber = nodegit::safeGetField(inputObj, "flags");
          if (!maybeNumber.IsEmpty() && !maybeNumber->IsUndefined() && !maybeNumber->IsNull()) {
            if (!maybeNumber->IsNumber()) {
              return {
                "Must pass Int32 to flags"
              };
            }

            output->raw->flags = static_cast<uint32_t>(maybeNumber->Int32Value(Nan::GetCurrentContext()).FromJust());
          }
        }
           {
          v8::Local<v8::Value> maybeNumber = nodegit::safeGetField(inputObj, "mode");
          if (!maybeNumber.IsEmpty() && !maybeNumber->IsUndefined() && !maybeNumber->IsNull()) {
            if (!maybeNumber->IsNumber()) {
              return {
                "Must pass Int32 to mode"
              };
            }

            output->raw->mode = static_cast<uint32_t>(maybeNumber->Int32Value(Nan::GetCurrentContext()).FromJust());
          }
        }
          output->raw->workdir_path = nullptr;
        {
          v8::Local<v8::Value> maybeString = nodegit::safeGetField(inputObj, "workdirPath");
          if (!maybeString.IsEmpty() && !maybeString->IsUndefined() && !maybeString->IsNull()) {
            if (!maybeString->IsString()) {
              return {
                "Must pass string to workdirPath"
              };
            }

            Nan::Utf8String utf8String(maybeString.As<v8::String>());
            output->raw->workdir_path = strdup(*utf8String);
          }
        }
          output->raw->description = nullptr;
        {
          v8::Local<v8::Value> maybeString = nodegit::safeGetField(inputObj, "description");
          if (!maybeString.IsEmpty() && !maybeString->IsUndefined() && !maybeString->IsNull()) {
            if (!maybeString->IsString()) {
              return {
                "Must pass string to description"
              };
            }

            Nan::Utf8String utf8String(maybeString.As<v8::String>());
            output->raw->description = strdup(*utf8String);
          }
        }
          output->raw->template_path = nullptr;
        {
          v8::Local<v8::Value> maybeString = nodegit::safeGetField(inputObj, "templatePath");
          if (!maybeString.IsEmpty() && !maybeString->IsUndefined() && !maybeString->IsNull()) {
            if (!maybeString->IsString()) {
              return {
                "Must pass string to templatePath"
              };
            }

            Nan::Utf8String utf8String(maybeString.As<v8::String>());
            output->raw->template_path = strdup(*utf8String);
          }
        }
          output->raw->initial_head = nullptr;
        {
          v8::Local<v8::Value> maybeString = nodegit::safeGetField(inputObj, "initialHead");
          if (!maybeString.IsEmpty() && !maybeString->IsUndefined() && !maybeString->IsNull()) {
            if (!maybeString->IsString()) {
              return {
                "Must pass string to initialHead"
              };
            }

            Nan::Utf8String utf8String(maybeString.As<v8::String>());
            output->raw->initial_head = strdup(*utf8String);
          }
        }
          output->raw->origin_url = nullptr;
        {
          v8::Local<v8::Value> maybeString = nodegit::safeGetField(inputObj, "originUrl");
          if (!maybeString.IsEmpty() && !maybeString->IsUndefined() && !maybeString->IsNull()) {
            if (!maybeString->IsString()) {
              return {
                "Must pass string to originUrl"
              };
            }

            Nan::Utf8String utf8String(maybeString.As<v8::String>());
            output->raw->origin_url = strdup(*utf8String);
          }
        }
    
  return {
    output
  };
}

                 
// force base class template instantiation, to make sure we get all the
// methods, statics, etc.
template class nodegit::ConfigurableClassWrapper<GitRepositoryInitOptionsTraits>;
